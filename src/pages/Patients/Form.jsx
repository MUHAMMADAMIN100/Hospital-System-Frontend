...
export default function NewPatientPage() {
  const navigate = useNavigate() // заменяем useRouter
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true)
      setTimeout(() => navigate("/patients"), 1500) // заменяем router.push
    } catch (err) {
      ...
      <Container maxWidth="md">
        ...
        <Box sx={{ mb: 4, p: 4, background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", borderRadius: 3, boxShadow: "0 8px 32px rgba(102, 126, 234, 0.3)" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
            ...
            <Typography variant="h3" sx={{ fontWeight: 700, color: "white" }}>Добавить пациента</Typography>
          </Box>
          <Typography variant="body1" sx={{ color: "rgba(255,255,255,0.9)" }}>Заполните форму для регистрации нового пациента</Typography>
        </Box>
